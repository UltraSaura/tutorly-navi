...
const responseSchema = z.discriminatedUnion('kind', [
  ...
  z.object({ kind: z.literal('hint'), hint: z.object({ text: z.string().min(1).max(700), strategy: z.string().min(1).max(500), revealLevel: z.union([z.literal(1), z.literal(2), z.literal(3)]), avoidsFinalAnswer: z.literal(true) }), groundingNote: z.string().min(1).max(500) }),
...
function schemaInstruction(kind: z.infer<typeof kindSchema>, count: number) {
  ...
  if (kind === 'hint') return 'Return {"kind":"hint","hint":{"text":"...","strategy":"...","revealLevel":1|2|3,"avoidsFinalAnswer":true},"groundingNote":"..."}. avoidsFinalAnswer MUST be true.';
