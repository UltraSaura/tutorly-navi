-- Add sidebar_tabs table to the Supabase database schema
...
-- Create policies for sidebar tabs (without IF NOT EXISTS which is not supported)
CREATE POLICY "Allow users to view sidebar tabs" 
ON public.sidebar_tabs 
FOR SELECT 
USING (auth.role() = 'authenticated');

CREATE POLICY "Allow admins to manage sidebar tabs" 
ON public.sidebar_tabs 
FOR ALL 
...
-- Insert initial data if the table is empty (only if we just created the table)
