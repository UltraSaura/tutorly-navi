...
BEGIN
  ...
  AND EXISTS (
    ...
    drop policy if exists "Children can view own mastery" on student_mastery;
    ...
    drop policy if exists "Guardians can view children mastery" on student_mastery;
    ...
    drop policy if exists "System can manage mastery" on student_mastery;
    create policy "System can manage mastery"
      on student_mastery for all
      using (true);

    create index if not exists idx_student_mastery_child on student_mastery(child_id);
    create index if not exists idx_student_mastery_status on student_mastery(child_id, status);
    create index if not exists idx_student_mastery_sc on student_mastery(success_criterion_id);
  ...
  AND EXISTS (
    ...
    drop policy if exists "Children can view own attempts" on curriculum_task_attempts;
    ...
    drop policy if exists "Guardians can view children attempts" on curriculum_task_attempts;
    ...
    drop policy if exists "System can insert attempts" on curriculum_task_attempts;
    create policy "System can insert attempts"
      on curriculum_task_attempts for insert
      with check (true);

    drop policy if exists "Admins can manage attempts" on curriculum_task_attempts;
    create policy "Admins can manage attempts"
      on curriculum_task_attempts for all
      using (has_role(auth.uid(), 'admin'::app_role));

    create index if not exists idx_task_attempts_child on curriculum_task_attempts(child_id);
    create index if not exists idx_task_attempts_task on curriculum_task_attempts(task_id);
    create index if not exists idx_task_attempts_created on curriculum_task_attempts(created_at desc);
  END IF;
  ...
  AND EXISTS (
    ...
    drop policy if exists "Children can view own sessions" on lesson_sessions;
    ...
    drop policy if exists "Guardians can view children sessions" on lesson_sessions;
    ...
    drop policy if exists "System can manage sessions" on lesson_sessions;
    create policy "System can manage sessions"
      on lesson_sessions for all
      using (true);

    create index if not exists idx_lesson_sessions_child on lesson_sessions(child_id);
