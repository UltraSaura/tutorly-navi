...
CREATE TABLE public.exercise_explanations_cache (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE INDEX idx_exercise_history_created_at ON public.exercise_history(created_at DESC);
CREATE INDEX idx_exercise_explanations_cache_hash ON public.exercise_explanations_cache(exercise_hash);
CREATE INDEX idx_exercise_explanations_cache_usage ON public.exercise_explanations_cache(usage_count DESC);
CREATE INDEX idx_exercise_attempts_history_id ON public.exercise_attempts(exercise_history_id);
