...
export default function TeacherTopicDetail() {
  ...
  if (!topic) {
    return <div className="p-4">{ui("Topic not found")}</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <PageMeta title={ui("Topic Details")} description={ui("Class-level performance breakdown for a specific learning topic.")} />
      {/* Header */}
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-muted-foreground">
            {subjectLabel(subject?.name)} • {topic.curriculum_level_code}
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>{ui("Topic Information")}</CardTitle>
        </CardHeader>
        <CardContent className="space-y-2">
          ...
          <div className="flex gap-4 text-sm text-muted-foreground">
            <span>{ui("Duration:")} {topic.estimated_duration_minutes} min</span>
            <span>{ui("Videos:")} {topic.video_count}</span>
            <span>{ui("Quizzes:")} {topic.quiz_count}</span>
          </div>
        </CardContent>
      </Card>
          ...
          <Card className="border-dashed">
            <CardContent className="pt-6 text-center">
              ...
              <p className="text-muted-foreground mb-4">
                {ui("No lesson content generated yet")}
              </p>
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground">
            {ui("Students can view this topic at:")}{' '}
