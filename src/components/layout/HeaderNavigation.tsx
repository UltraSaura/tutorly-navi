...
const LanguageMenuItems = () => {
  ...
  const languages = [{ code: 'en', name: t('language.english'), flag: '🇺🇸' }, { code: 'fr', name: t('language.french'), flag: '🇫🇷' }];
  const manuallySet = localStorage.getItem('languageManuallySet') === 'true';
  return <>{!manuallySet && <><DropdownMenuItem disabled className="text-xs text-muted-foreground">{t('language.autoDetected')}</DropdownMenuItem><DropdownMenuSeparator /></>}{languages.map((lang) => <DropdownMenuItem key={lang.code} onClick={() => { i18n.changeLanguage(lang.code); localStorage.setItem('lang', lang.code); localStorage.setItem('languageManuallySet', 'true'); window.location.reload(); }} className={i18n.resolvedLanguage === lang.code ? 'bg-accent' : ''}><span className="mr-2">{lang.flag}</span>{lang.name}{i18n.resolvedLanguage === lang.code && !manuallySet && <span className="ml-auto text-xs text-muted-foreground">{t('language.auto')}</span>}</DropdownMenuItem>)}</>;
};
...
export function HeaderNavigation() {
  ...
  return <div className="flex items-center justify-between w-full h-full px-6">
    ...
    <div className="hidden md:flex items-center gap-3"><AdminPreviewSelector /><DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" className="flex items-center gap-2 h-auto p-2 hover:bg-accent/50"><Avatar className="h-8 w-8"><AvatarFallback className="text-sm">{userInitials}</AvatarFallback></Avatar><div className="flex flex-col items-start text-sm"><span className="text-xs text-muted-foreground truncate max-w-32">{user?.email}</span></div><ChevronDown className="h-4 w-4" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-56 bg-background border shadow-md">
      ...
      <DropdownMenuItem onClick={handleSignOut} disabled={isSigningOut} className="text-destructive focus:text-destructive"><LogOut className="mr-2 h-4 w-4" /><span>{isSigningOut ? ui("Signing out...") : ui("Sign Out")}</span></DropdownMenuItem>
