...
const ExerciseList = ({
  ...
  const {
    t,
    i18n
  } = useTranslation(); // <-- Updated hook usage
  ...
  const handleShowExplanation = (exerciseId: string) => {
    ...
    if (exercise) {
      ...
      teaching.openFor(exerciseRow, {
        response_language: mapLanguageForAI(i18n.resolvedLanguage),
        // <-- Updated
  ...
  const convertToExerciseCard = (exercise: ExerciseType) => ({
    ...
    // Default to math, should be determined from exercise data
    prompt: exercise.question,
    userAnswer: exercise.userAnswer,
    explanation: exercise.explanation,
    status: exercise.isCorrect === undefined ? 'unanswered' as const : exercise.isCorrect ? 'correct' as const : 'incorrect' as const
  });
  return <div className="flex flex-col h-full">
      ...
      <div className="px-6 py-4 border-b border-neutral-border bg-neutral-surface">
        <div className="max-w-6xl mx-auto space-y-2">
          ...
          <div className="flex items-center justify-between">
            ...
            {exercises.length > 0 && onClearExercises && <Button variant="ghost" size="sm" onClick={onClearExercises} className="text-neutral-muted hover:text-neutral-text">
                <Trash2 size={16} />
              </Button>}
          </div>
          ...
          <div className="flex items-center gap-3">
            <XpChip level={userStats.currentLevel} xp={userStats.currentXp} />
            <CompactStreakChip days={userStats.streakDays} active={userStats.streakActive} />
      ...
      <div className="flex-1 overflow-hidden">
        <ScrollArea className="h-full">
          <div className="p-6">
            {exercises.length === 0 ? (/* Empty State */
          <div className="flex flex-col items-center justify-center h-full text-center py-12">
                <div className="w-full mb-8 flex items-center justify-center">
                  <img src="/images/student-illustration.png" alt="Student with laptop and books" className="w-full max-w-2xl h-auto object-contain" />
                </div>
              ...
              </div>) : (/* Exercise Cards Grid */
          <div className="max-w-6xl mx-auto">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {exercises.map(exercise => <ExerciseCard key={exercise.id} {...convertToExerciseCard(exercise)} onShowExplanation={handleShowExplanation} onTryAgain={handleTryAgain} onSubmitAnswer={onSubmitAnswer} />)}
                </div>
              </div>)}
          </div>
        </ScrollArea>
      ...
      {teaching.open && <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-2 md:p-4 z-50">
          <div className="w-full max-w-2xl rounded-2xl bg-card border border-border shadow-lg max-h-[95vh] md:max-h-[80vh] overflow-hidden flex flex-col">
            <div className="flex items-center justify-between p-4 md:p-5 border-b border-border">
              ...
              <Button variant="ghost" size="icon" onClick={() => teaching.setOpen(false)} className="h-6 w-6" aria-label="Close">
                <span className="text-sm">✖</span>
              </Button>
            </div>

            <div className="flex-1 overflow-y-auto p-4 md:p-5">
              {teaching.loading ? <div className="space-y-3">
                  <div className="h-16 rounded-xl bg-muted animate-pulse" />
                  <div className="h-16 rounded-xl bg-muted animate-pulse" />
                </div> : teaching.sections ? <TwoCards s={teaching.sections} /> : <p className="text-sm text-destructive">{teaching.error || "No explanation yet."}</p>}
            </div>

            <div className="p-4 md:p-5 border-t border-border">
              <Button onClick={() => teaching.setOpen(false)} className="w-full" size="lg">
