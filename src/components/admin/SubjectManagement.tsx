...
const SubjectManagement = () => {
  ...
  return (
    <div className="space-y-6">
      <PageMeta title={ui("Subjects")} description={ui("Manage subjects available across the Stuwy platform.")} />
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">{ui("Subject Management")}</h1>
          <p className="text-muted-foreground mt-1">
            {ui("Manage subjects for homework, document analysis, and learning platform")}
          </p>
        </div>
      </div>
      ...
      <Tabs defaultValue="chat-subjects" className="space-y-6">
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="chat-subjects">{ui("Chat/Homework Subjects")}</TabsTrigger>
          <TabsTrigger value="learning-subjects">{ui("Learning Platform Subjects")}</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="chat-subjects" className="space-y-6">
              ...
              <Dialog open={showAddCategoryDialog} onOpenChange={setShowAddCategoryDialog}>
                <DialogTrigger asChild>
                  <Button variant="outline" className="flex gap-2">
                    <FolderPlus className="h-4 w-4" /> {ui("Add Category")}
                  </Button>
                </DialogTrigger>
                <DialogContent className="sm:max-w-[500px]">
                  <DialogHeader>
                    <DialogTitle>{ui("Add New Category")}</DialogTitle>
                    <DialogDescription>
                      {ui("Create a new category to organize your subjects.")}
                    </DialogDescription>
                  </DialogHeader>
                  
                  <div className="grid gap-4 py-4">
                    <div className="space-y-2">
                      <Label htmlFor="categoryName">{ui("Category Name")}</Label>
                      <Input
                        id="categoryName"
                        placeholder={ui("e.g., Science")}
                  ...
                  <DialogFooter>
                    <Button variant="outline" onClick={() => setShowAddCategoryDialog(false)}>{ui("Cancel")}</Button>
              ...
              <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
                <DialogTrigger asChild>
                  <Button className="bg-stuwy-600 hover:bg-stuwy-700">
                    <Plus className="mr-2 h-4 w-4" /> {ui("Add Subject")}
                  </Button>
                </DialogTrigger>
                <DialogContent className="sm:max-w-[500px]">
                  <DialogHeader>
                    <DialogTitle>{ui("Add New Subject")}</DialogTitle>
                    <DialogDescription>
                      {ui("Add a new subject for homework and document analysis.")}
                    </DialogDescription>
                  </DialogHeader>
                  
                  <div className="grid gap-4 py-4">
                    <div className="space-y-2">
                      <Label htmlFor="subjectName">{ui("Subject Name")}</Label>
                      <Input
                        id="subjectName"
                        placeholder={ui("e.g., Physics")}
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="subjectDesc">{ui("Description (Optional)")}</Label>
                      <Input
                        id="subjectDesc"
                        placeholder={ui("Brief description")}
          ...
          <div className="mt-4">
            <div className="mb-2 flex items-center gap-2">
              <h2 className="text-lg font-medium">{ui("Categories")}</h2>
              <Badge className="ml-1 text-xs">{categories.length}</Badge>
              <span className="text-sm text-muted-foreground ml-auto">{ui("Drag subjects to reorder or move between categories")}</span>
            </div>
