    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="relative">
          ...
          <Input
            placeholder={ui("Search by country or level name...")}
            value={searchFilter}
        ...
        <Input
          type="number"
          placeholder={ui("Filter by age (e.g., 10)")}
          min="0"
          max="18"
          value={ageFilter}
      ...
      <div className="flex items-start gap-2 p-3 bg-blue-50 dark:bg-blue-950 rounded-md border border-blue-200 dark:border-blue-800">
        ...
        <p className="text-sm text-blue-800 dark:text-blue-200">
          {ui("Select school levels by checking boxes. Each level shows its approximate age range. Selected levels are stored as")} <code className="text-xs bg-blue-100 dark:bg-blue-900 px-1 rounded">country_code:level_code</code>.
        </p>
      </div>
      ...
      <ScrollArea className="h-[500px] border rounded-md">
        <Table>
          <TableHeader className="sticky top-0 bg-background z-10">
            <TableRow>
              <TableHead className="w-[200px]">{ui("Country")}</TableHead>
              <TableHead>{ui("School Levels (with age ranges)")}</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {filteredCountries.length === 0 ? (
              <TableRow>
                <TableCell colSpan={2} className="text-center text-muted-foreground py-8">
                  {ui("No school levels found matching your filters.")}
                </TableCell>
              </TableRow>
            ) : (
                          ...
                          >
                            ...
                            >
                              ...
                              <Badge variant="outline" className="text-xs">
                                {ui("Ages")} {level.ageRange[0]}-{level.ageRange[1]}
                              </Badge>
                            </Label>
                          </div>
                        );
